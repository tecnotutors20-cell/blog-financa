...
item=json.loads((root/'content/automation/2026-08-30-1900.json').read_text())
...
for cluster in data['clusters']:
    if cluster['name']=='Segurança financeira':
        ...
        cluster['next_long_tails']=[x for x in cluster['next_long_tails'] if x!='Fiz um Pix após cair em golpe: quais passos tomar imediatamente?']
c.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
pub=root/'scripts/publish_blogger.py'; s=pub.read_text(); key='fiz um pix após cair em golpe: o que fazer imediatamente para tentar recuperar o dinheiro?'
if key not in s:
    marker='COVER_BY_TITLE = {'
    s=s.replace(marker, marker+'\n    "'+key+'": f"{COVER_BASE}/pix-golpe-o-que-fazer-20260830.svg",')
    pub.write_text(s)
