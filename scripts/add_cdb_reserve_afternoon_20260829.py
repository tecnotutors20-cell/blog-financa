...
item=json.loads((root/'content/automation/2026-08-29-1400.json').read_text())
...
for cluster in data['clusters']:
    if cluster['name']=='Reserva e renda fixa':
        ...
        cluster['next_long_tails']=[x for x in cluster['next_long_tails'] if x!='CDB com liquidez diária serve para reserva de emergência?']
...
if key not in s:
    ...
    s=s.replace(marker, marker+'\n    "'+key+'": f"{COVER_BASE}/cdb-reserva-emergencia-20260829.svg",')
    pub.write_text(s)
