...
item=json.loads((root/'content/automation/2026-08-27-0900.json').read_text())
...
for cluster in data['clusters']:
    if cluster['name']=='Dívidas':
        ...
        cluster['next_long_tails']=['Vale a pena pegar empréstimo para pagar cartão de crédito?','Como priorizar dívidas quando não dá para pagar todas?','Dívida negociada pode voltar se eu atrasar o acordo?']
c.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
pub=root/'scripts/publish_blogger.py'; s=pub.read_text(); key='renegociação de dívidas: como negociar sem trocar uma dívida por outra pior'
if key not in s:
    marker='COVER_BY_TITLE = {'
    s=s.replace(marker, marker+'\n    "'+key+'": f"{COVER_BASE}/renegociacao-dividas-guia-20260827.svg",')
    pub.write_text(s)
