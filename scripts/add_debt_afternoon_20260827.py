...
item=json.loads((root/'content/automation/2026-08-27-1400.json').read_text())
...
for cluster in data['clusters']:
    if cluster['name']=='Dívidas':
        ...
        cluster['next_long_tails']=[x for x in cluster['next_long_tails'] if x!='Vale a pena pegar empréstimo para pagar cartão de crédito?']
c.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
pub=root/'scripts/publish_blogger.py'; s=pub.read_text(); key='vale a pena pegar empréstimo para pagar cartão de crédito? veja como comparar'
if key not in s:
    marker='COVER_BY_TITLE = {'
    s=s.replace(marker, marker+'\n    "'+key+'": f"{COVER_BASE}/emprestimo-pagar-cartao-20260827.svg",')
    pub.write_text(s)
