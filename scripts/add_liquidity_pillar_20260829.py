...
item=json.loads((root/'content/automation/2026-08-29-0900.json').read_text())
...
for cluster in data['clusters']:
    if cluster['name']=='Reserva e renda fixa':
        ...
        cluster['next_long_tails']=['CDB com liquidez diária serve para reserva de emergência?','Tesouro Selic pode perder dinheiro se resgatar antes?','Quanto guardar por mês para formar R$ 10 mil de reserva?']
c.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
pub=root/'scripts/publish_blogger.py'; s=pub.read_text(); key='liquidez na renda fixa: o que significa e quando você consegue resgatar o dinheiro'
if key not in s:
    marker='COVER_BY_TITLE = {'
    s=s.replace(marker, marker+'\n    "'+key+'": f"{COVER_BASE}/liquidez-renda-fixa-20260829.svg",')
    pub.write_text(s)
