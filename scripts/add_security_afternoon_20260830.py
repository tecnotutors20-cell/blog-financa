...
item=json.loads((root/'content/automation/2026-08-30-1400.json').read_text())
...
for cluster in data['clusters']:
    if cluster['name']=='Segurança financeira':
        ...
        cluster['next_long_tails']=[x for x in cluster['next_long_tails'] if x!='Como identificar falso atendimento de banco no WhatsApp?']
c.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
pub=root/'scripts/publish_blogger.py'; s=pub.read_text(); key='como identificar falso atendimento de banco no whatsapp? 9 sinais de golpe'
if key not in s:
    marker='COVER_BY_TITLE = {'
    s=s.replace(marker, marker+'\n    "'+key+'": f"{COVER_BASE}/falso-atendimento-banco-whatsapp-20260830.svg",')
    pub.write_text(s)
